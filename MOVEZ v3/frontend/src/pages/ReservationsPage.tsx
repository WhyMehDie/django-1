...
import { fetchTerrains, fetchMyReservations, cancelReservation, createReservation } from '../features/reservations/reservationsSlice';
import type { Terrain } from '../features/reservations/reservationsSlice';
...
export default function ReservationsPage() {
  ...
  const handleBook = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!selectedTerrain || !date || !startTime || !endTime) return;

    try {
      await dispatch(createReservation({
        reservation_type: 'TERRAIN',
        terrain: selectedTerrain.id,
        date,
      ...
      alert('Erreur lors de la réservation. Veuillez vérifier les disponibilités.');
    }
  };
    ...
    <div className="page-container">
      ...
      {selectedTerrain && (
        <div className="res-modal-overlay">
          <div className="res-modal glass">
            <h2>Réserver: {selectedTerrain.name}</h2>
            <p className="res-modal-subtitle">{selectedTerrain.sport} — {selectedTerrain.price_per_hour} MAD/h</p>
            ...
            <form onSubmit={handleBook} className="res-form">
              ...
              <div className="res-modal-actions">
                <button type="button" className="btn-secondary" onClick={() => setSelectedTerrain(null)}>Annuler</button>
              ...
              {terrains.map((t) => (
                <div key={t.id} className="terrain-card">
                  ...
                  <div className="terrain-body">
                    ...
                    <button className="btn-primary" style={{ marginTop: '12px' }} onClick={() => setSelectedTerrain(t)}>
                      Réserver
                    </button>
                  </div>
                </div>
          ...
          {reservations.length === 0 ? (
            ...
            reservations.map((r) => (
              <div key={r.id} className="reservation-row">
                <div className="res-type">{r.reservation_type === 'TERRAIN' ? 'Terrain' : 'Coaching'}</div>
